...
# Book configuration: (level, puzzle_type, args, seed)
...
def generate_ten_columns(level: int, seed: int, prev_link: str, next_link: str) -> bool:
...
def main():
    ...
    for level, puzzle_type, args, seed in LEVELS:
        ...
        if puzzle_type == "ten-columns":
            success = generate_ten_columns(level, seed, prev_link, next_link)
            if not success:

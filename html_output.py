...
def generate_ten_columns_html(
    grid_data: list[list[dict]],
    title: str = "Number Crosswords",
    subtitle: str = "",
    show_solution_button: bool = True,
    prev_link: str = "",
    next_link: str = "",
...
<body>
    ...
    <div class="puzzle-container">
        ...
        <div class="info">
            <p>Fill in the blanks so each column forms a valid equation.</p>
            <p>Each digit 0-9 is used exactly once.</p>
        </div>
    ...
    <script>
        ...
        function clearInputs() {{
            ...
            inputs.forEach(input => {{
                input.value = '';
                input.parentElement.classList.remove('correct', 'incorrect', 'filled');
            }});
            updateKeyboardHighlights();
            if (inputs.length > 0) {{
                inputs[0].focus();
        ...
        function showSolution() {{
            ...
            inputs.forEach(input => {{
                ...
                const col = parseInt(input.dataset.col);
                input.value = gridData[row][col].answer;
                input.parentElement.classList.add('correct', 'filled');
                input.parentElement.classList.remove('incorrect');
            }});
            updateKeyboardHighlights();
        }}
...
def generate_crossword_digits_html(
    grid_data: list[list[dict]],
    title: str = "Number Crosswords",
    subtitle: str = "",
    show_solution_button: bool = True,
...
def save_crossword_digits_html(
    grid_data: list[list[dict]],
    path: str,
    title: str = "Number Crosswords",
    subtitle: str = "",
